...
func make_case(spec, index):
	var source = load(spec[1]).instance()
	var source_body = source.get_node(spec[2])
	var source_shape = source_body.get_node(spec[3])
	...
	rectangle.extents = Vector2(20, 20)
	...
	cases.append({"name":spec[0], "axis":spec[4], "origin":origin, "wall":wall, "mover":mover, "shape_size":wall_shape.shape.extents * 2.0})
	source.free()

func emit_results():
	for item in cases:
		var coordinate = item.mover.position.x - item.origin.x if item.axis == "x" else item.mover.position.y - item.origin.y

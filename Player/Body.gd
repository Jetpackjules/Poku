...
var proximity_threshold := deg_to_rad(45)
@export var starts_flipped := false

func flip_orientation():

	new_desired_angle += deg_to_rad(180)
...
func set_color(body_color, limbs_color):
	var limb_style = spine1.get_node("Panel").get("theme_override_styles/panel")
	var body_style = get_node("Panel").get("theme_override_styles/panel")
...
#			part.get_node("PinJoint2D").softness = 1
			...
			part.set_collision_mask_value(3, true)
			part.set_collision_layer_value(2, true)
			part.set_collision_layer_value(3, true)
...
#			print(test)
		...
		for part in spine:
			...
			part.set_collision_mask_value(3, false)
			part.set_collision_layer_value(2, false)
			part.set_collision_layer_value(3, false)
...
#			grabbed_item.rotation = deg2rad(180)
			grabbed_item.new_desired_angle = deg_to_rad(180)
...
#		# Upper Leg power
		...
		Leg_R_up.new_desired_angle = deg_to_rad(15)
		Leg_L_up.new_desired_angle = deg_to_rad(-15)

		Leg_R_down.new_desired_angle = deg_to_rad(5)
		Leg_L_down.new_desired_angle = deg_to_rad(-5)
...
func win(status: bool) -> void:
	...
	if status and !trophy:
		trophy = load("res://Items/Trophy.tscn").instantiate()
